...
const Wishlist = () => {
    ...
    return (
      <div className='max-w-7xl mx-auto px-4 py-20 text-center space-y-4'>
        ...
        <p className='text-sm text-zinc-500 max-w-sm mx-auto'>
          Save your favorite jewellery pieces here to view them later.
        </p>
        <Link
          to='/products'
  ...
  return (
    <div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10'>
      ...
      <p className='text-sm text-zinc-500 mb-8'>Your saved precious items ({wishlistProducts.length})</p>

      <div className='grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-6'>
        {wishlistProducts.map((product) => (
          <div
            key={product.id}
            className='group bg-white border border-zinc-200 rounded-2xl overflow-hidden flex flex-col justify-between p-3 relative'
          >
            <button
              onClick={() => removeFromWishlist(product.id)}
              className='absolute top-5 right-5 z-10 p-2 bg-white/80 backdrop-blur-md rounded-full text-zinc-400 hover:text-red-600 transition-colors cursor-pointer'
              title='Remove'
            >
